...
async function start() {
  ...
  const onCheck = async (phone, command, args) => {
    ...
    if (userStates[phone]) {
      ...
      const isMenuCommand = ["menu", "help", "start"].includes(
        command.toLowerCase(),
      );
      ...
      if (!isNumericCommand && !isMenuCommand) {
        ...
        console.log(
          `[Chatbot] Received parameter: [${args}] for pending command [${command}]`,
        );
      } else {
    ...
    let user = users.find((u) => {
      ...
      // Khớp nếu số điện thoại này nằm trong số kia (xử lý mã quốc gia)
      return normalizedIncoming.endsWith(normalizedDB) || normalizedDB.endsWith(normalizedIncoming);
    });

    if (!user) {
      console.log(`[Auth Debug] Incoming: ${normalizedIncoming} not found in DB:`, users.map(u => u.phone.replace(/\D/g, "")));
    }
    ...
    if (user) {
      ...
      console.log(`[Auth] Checking teacher list from Sheets for: ${phone}`);
      const admins = users.filter((u) => u.role === "ADMIN" && u.sheetId);

      for (const admin of admins) {
        try {
          const tempSheets = new SheetsClient(admin.sheetId);
          const teachersRaw = await tempSheets.getRange(
          ...
          const match = teachers.find((t) => {
            ...
            return (
              sheetPhone.length >= 9 &&
              incomingPhone.endsWith(
                sheetPhone.startsWith("0")
                  ? sheetPhone.substring(1)
                  : sheetPhone,
              )
            );
          });
          ...
          if (match) {
            ...
            console.log(
              `[Auth] ✅ Teacher recognized: ${teacherName} (from Admin ${admin.name}'s sheet)`,
            );
            break;
          }
        } catch (e) {
    ...
    if (command !== "register" && !sheetId) {
      return "❌ Google Sheet ID not set. Please use command: register [Sheet_ID]";
    }
    ...
    try {
      if (command === "import_media_base64") {
        try {
          const tempFilePath = path.join(
            __dirname,
            `temp_import_${Date.now()}.xlsx`,
          );
          fs.writeFileSync(tempFilePath, Buffer.from(args, "base64"));

          const { execFileSync } = require("child_process");
          console.log(
            `[App] ⚙️ Uploading & Converting Excel to Google Sheets...`,
          );

          const now = new Date();
          const timestamp = now.toLocaleDateString().replace(/\//g, '-') + '_' + now.toLocaleTimeString('en-GB').replace(/:/g, '');
          const uploadArgs = [
            "drive",
            "upload",
            tempFilePath,
            "--convert",
            `--name=Smart Timetable Data (${timestamp})`,
            "--json",
          ];
          if (CONFIG.DRIVE_FOLDER_ID) {
            uploadArgs.push(`--parent=${CONFIG.DRIVE_FOLDER_ID}`);
          }

          const resultRaw = execFileSync("gog", uploadArgs, {
            encoding: "utf8",
          });
      ...
      if (command === "params") {
        ...
        const constraintsRaw = await sheets.getRange(
        ...
        constraints.forEach((c) => {
          report += `  • ${c[0]} | ${c[1]} ${c[2]}-${c[3]} | ${c[4] || "N/A"}\n`;
        });

        const bandsRaw = await sheets.getRange(
          CONFIG.SHEET_RANGES.BANDED_GROUPS,
        );
        const bands = bandsRaw.slice(1);
        report += "\n*2. Banded Groups:*\n";
        if (bands.length === 0) report += "  (No banded groups)\n";
        bands.forEach((b) => {
          report += `  • Group ${b[0]}: ${b[1]} | Teachers: ${b[2]} | Classes: ${b[3]}\n`;
        });
      ...
      if (command === "check") {
        ...
        if (result.success)
          return "✅ Perfect! No timetable conflicts detected.";
        ...
        if (result.loadErrors.length > 0)
          report +=
            ...
            "\n\n";
        if (result.constraintErrors.length > 0)
          report +=
            "🚫 *BLOCKED SLOTS:*\n" +
            result.constraintErrors.map((s) => `• ${s}`).join("\n") +
            "\n\n";
        if (result.bandErrors.length > 0)
          report +=
            "🖇️ *BANDED GROUPS:*\n" +
            result.bandErrors.map((s) => `• ${s}`).join("\n") +
            "\n\n";
        if (result.roomErrors.length > 0)
          report +=
            "🏠 *ROOM CONFLICTS:*\n" +
            result.roomErrors.map((s) => `• ${s}`).join("\n") +
            "\n\n";
        if (result.recessErrors && result.recessErrors.length > 0)
          report +=
            "☕ *RECESS VIOLATIONS:*\n" +
            result.recessErrors.map((s) => `• ${s}`).join("\n") +
            "\n\n";
        if (result.deployIssues && result.deployIssues.length > 0)
          report +=
            "🚫 *DEPLOYMENT ISSUES:*\n" +
            result.deployIssues.map((s) => `• ${s}`).join("\n") +
            "\n\n";
        if (result.deptIssues && result.deptIssues.length > 0)
          report +=
            "👥 *DEPARTMENT PLT ISSUES:*\n" +
      ...
      if (command === "sync") {
        ...
        return (
          ...
          `📧 Target Calendar: ${syncResult.masterCalendar}\n\n` +
          `🔗 *Click to view:* https://calendar.google.com/`
        );
      }

      if (command === "import") {
        return `📥 *IMPORT DATA FROM EXCEL*\n──────────────────\n\n_Please attach an Excel (.xlsx) or CSV file to this message and send it to the bot._\n\n*The system will automatically update the data!*`;
      }
      ...
      if (command === "swap") {
        ...
        if (slot1Idx !== -1) {
          ...
          msg += `• From ${day1} ${time1} → ${day2} ${time2}\n`;
        }
        if (slot2Idx !== -1) {
          timetable[slot2Idx][1] = day1;
          timetable[slot2Idx][2] = time1;
          msg += `• From ${day2} ${time2} → ${day1} ${time1}\n`;
        }

        // Ghi lại
        await sheets.updateRange(CONFIG.SHEET_RANGES.TIMETABLE, [
          timetableRaw[0],
          ...timetable,
        ]);
        return (
      ...
      if (command === "schedule") {
        ...
        if (!target && role === "ADMIN") {
          ...
          return `👋 Hello Admin! Whose schedule would you like to view?\n👉 *Please type the Teacher Name below:*`;
        }
        const finalTarget = target || teacherName || "Admin";
        const slots = await scheduler.getSchedule(finalTarget);
        if (!slots)
          return `ℹ️ No schedule found for "${finalTarget}".\n👉 Type *stats* to see the teacher list!`;
      ...
      if (command === "rooms") {
        ...
        if (!day || !time)
          return "👉 Please enter: rooms [Day] [Time] (Example: rooms Mon 09:00)";
        const available = await scheduler.getAvailableRooms(day, time);
        if (available.length === 0)
          return `❌ No empty rooms found at ${day} ${time}.`;
        return (
          `🏠 *AVAILABLE ROOMS (${day} ${time}):*\n` +
      ...
      if (command === "generate") {
        ...
        const genResult = isBest
          ? await generator.generateBest(20)
          : await generator.generate();

        let report = `🤖 *AI GENERATION RESULT ${isBest ? "(OPTIMIZED)" : ""}*\n──────────────────\n`;
        report += `✅ Placed: ${genResult.totalPlaced} slots\n`;
        if (isBest)
          report += `⭐ Quality Score: ${genResult.score.toFixed(2)}\n`;
        ...
        if (genResult.failedCount > 0) {
          ...
          if (r.capacity > 0) {
            report += `  • Exceeded 40 slots/class: ${r.capacity} slots (Critical error!)\n`;
            report += `    👉 *Advice:* Check 'Deployment' tab. You might have cloned too many subjects for the same class (Total slots exceed weekly capacity).\n`;
          }
          if (r.class_busy > 0)
            report += `  • Class clash: ${r.class_busy} slots\n`;
          if (r.teacher_busy > 0)
            report += `  • Teacher clash: ${r.teacher_busy} slots\n`;
          if (r.blocked > 0)
            report += `  • Blocked slot violation: ${r.blocked} slots\n`;
          if (r.room_full > 0)
            report += `  • No empty rooms: ${r.room_full} slots\n`;
        } else {
      ...
      if (command === "relief") {
        ...
        if (isAdvance && !date) {
          return `📅 *ADVANCE RELIEF PLANNING*\n──────────────────\n\n_Plan relief for a future date._\n\n*Usage:*\nrelief advance [Absent_Day] [Timetable_Day]\n\n*Example:*\n• \`relief advance Wed\` → Plan for Wednesday\n• \`relief advance Wed Tue\` → Absent on Wed, use Tuesday schedule\n\n👉 After reviewing, type *relief confirm [Day]* to notify teachers.`;
        }

        const targetDay = date || "T2";
        const result = await manager.planRelief(targetDay, mappingDay);
        if (!result.plan || result.plan.length === 0)
          return result.message || "No relief plan needed.";
        if (isConfirm) {
          await manager.saveReliefPlan(result.plan);
          const teachersRaw = await sheets.getRange(
            CONFIG.SHEET_RANGES.TEACHERS,
          );
          const teacherMap = {};
        ...
        result.plan.forEach((p) => {
          report += `🔸 *${p.absentTeacher}* (Class ${p.className})\n   └ Slot: ${p.slot}\n   └ Covering: *${p.reliefTeacher}*\n\n`;
        });
      ...
      if (command === "save") {
        ...
        if (result.success)
          return `✅ Snapshot created successfully: *${result.name}*\n🔗 *Access Link:*\nhttps://docs.google.com/spreadsheets/d/${result.id}/edit`;
        return `❌ Error: ${result.error}`;
      }

      if (command === "backup" || command === "history") {
        const history = new HistoryManager();
        ...
        snapshots.slice(0, 10).forEach((s) => {
          text += `🕒 ${new Date(s.created).toLocaleString()}\n📝 ${s.name}\n🔗 ${s.url}\n\n`;
        });
        return text;
      }
      ...
      if (command === "matrix") {
        ...
        if (!table)
          return `ℹ️ Không tìm thấy dữ liệu ma trận cho "${finalTarget}".`;
        return table;
      }
      ...
      if (command === "book") {
        ...
        if (!room || !day || !start || !end)
          return "👉 Please enter: book [Room] [Day] [Start] [End] [Description]";

        // Conflict check
        const currentBookingsRaw = await sheets.getRange(
          CONFIG.SHEET_RANGES.BOOKINGS,
        );
        const currentTimetableRaw = await sheets.getRange(
          CONFIG.SHEET_RANGES.TIMETABLE,
        );
        const errors = await validator.checkRoomConflicts(
          currentTimetableRaw.slice(1),
          [
            ...(currentBookingsRaw ? currentBookingsRaw.slice(1) : []),
            [room, day, start, end, desc],
          ],
        );

        const hasConflict = errors.some(
          (e) => e.includes(room) && e.includes(day) && e.includes(start),
        );
        if (hasConflict)
          return `❌ Booking failed: ${room} is busy at this time!\n${errors.find((e) => e.includes(room))}`;

        const newBooking = [room, day, start, end, desc];
        const finalBookings = (
          currentBookingsRaw || [["Room", "Day", "Start", "End", "Description"]]
        ).concat([newBooking]);
      ...
      console.error(
        `[Command Error] Error processing ${command}:`,
        err.message,
      );
  ...
  app.get("/api/data", async (req, res) => {
    try {
      ...
      if (!admin || !admin.sheetId)
        return res.status(403).json({ error: "No Admin Sheet configured" });
      ...
      res.json({
        schoolName: "SMART-TIMETABLE SCHOOL",
  ...
  app.post("/api/command", async (req, res) => {
    ...
    try {
      ...
      const targetPhone =
        phone || (users.find((u) => u.role === "ADMIN") || {}).phone;
      if (!targetPhone) throw new Error("Target phone not specified.");

      console.log(
        `[Remote] Dashboard sent command: ${command} (${args || ""}) to ${targetPhone}`,
      );
  ...
  app.post("/webhook/trigger", async (req, res) => {
    console.log(`[Webhook] Signal from Sheet: ${req.body.sheetId}`);
    res.json({ success: true });
  });

  app.listen(3001, () =>
    console.log(
      `📡 Web Dashboard & Webhook running at: http://localhost:3001`,
    ),
  );
}
